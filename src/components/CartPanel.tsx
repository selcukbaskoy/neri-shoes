...
export default function CartPanel({ rates }: { rates: Record<string, number> }) {
  ...
  const { items, removeItem, updateQuantity, totalAmount, totalCount, isOpen, setOpen } =
    useCart();
            ...
            {items.length > 0 && (
              <div className="border-t border-[#222] px-5 py-5">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="w-full rounded-lg bg-accent px-6 py-3.5 text-sm font-bold tracking-[0.1em] text-[#0a0a0a] transition-all duration-200 hover:bg-accent/90 hover:shadow-[0_0_20px_rgba(255,208,0,0.4)]"
                >
                  {t("proceedToCheckout")}
                </motion.button>
